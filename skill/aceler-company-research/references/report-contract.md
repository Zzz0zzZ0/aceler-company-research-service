...
Assign independent 0–10 `product_match`, `commercial_match`, `follow_up` (`跟进`/`淘汰`), and rationale in the same call. Product measures evidenced process/catalog fit; commercial measures transaction, throughput, recurrence, and purchase/resale/supply/specification control. Normally both scores are at least 5. Commercial 4 may follow only when company evidence establishes recurring consumption/input, resale, material-inclusive delivery/procurement, specification control, or actionable complementary supply, with only scale/access unknown. Scores never trigger alone. Equipment/EPC/installation/customer access or peer/portfolio overlap without that action is no exception. Cite the action/evidence; neither dimension offsets the other.
