...
In the same call, assign independent 0–10 `product_match` and `commercial_match`, `follow_up` (`跟进` or `淘汰`), and a rationale. Product measures supported process/catalog fit; commercial measures the current transaction route, throughput, recurrence, and control of purchase, resale, supply, or specification. Normally both are at least 5 for follow-up. Commercial 4 may follow only when company evidence establishes recurring consumption/input, actual resale, material-inclusive delivery/procurement, specification control, or actionable complementary supply; only scale or access may remain unknown. Scores alone never trigger follow-up. Equipment/EPC/installation/customer access, peer status, or portfolio overlap without that action is not an exception. Cite the action and evidence; neither dimension offsets a weak other dimension.
