...
检索可靠性试验分支增加一轮有界补检：普通请求在无证据、外部/摘要降级、只有目录或社交来源、缺少产品/工艺页时复用现有语义检索。平台根页不能通过补检确认为目标主体时返回失败；其他补检未改善证据时保留原证据。该检测不决定产品相关性。缓存证据也经过质量检查，暂不把补检结果覆盖到旧缓存。试验结果通过验收后才合入 main。
