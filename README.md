...
检索可靠性试验分支增加一轮有界补检：普通请求在无证据、外部/摘要降级、只有目录或社交来源、缺少产品/工艺页时复用现有语义检索。平台根页不能通过补检确认为目标主体时返回失败；其他补检未改善证据时保留原证据。该检测不决定产品相关性。缓存证据也经过质量检查，暂不把补检结果覆盖到旧缓存。**2026-09-08 试验未通过验收，已退回 f8e38ed，不合入 main，请勿部署此试验分支。**

试验 100-3 完整运行 100 家、无人工补跑：100/100 有效，召回 82.14%、精确 76.67%、准确 76.00%，TP/FP/TN/FN 为 46/14/30/10；对照旧基线 98/100 有效、83.93%/83.93%/81.00%。14 家补检的判对数未净增加，86 家同原始证据的模型判断波动造成整体判对数减少 5；不能把所有下降归因于补检。新增 58 次检索 Agent 调用。另有独立合成探针复现两种检索质量字段不一致、补检缺口被误判关闭的问题。140 项现有回归通过不足以支持上线。原始结果保存在本地 `outputs/semantic-decision-validation/20260908T091844Z-testset100-3-retrieval-trial/`，审查及退回报告在 `outputs/retrieval-quality-trial-20260908/`；不继续 100-4 扩大测试。

CRM 批量脚本直接接受七列或八列 Markdown，网址列支持裸 HTTP(S) URL、自动链接和 Markdown 链接，并读取真实 href。检索成功或失败均保存脱敏的 `anysearch-meta.json`：包含阶段、调用计数、逐页提取错误，以及实际模块目录、Python/Node/CLI 路径和模型配置。验证报告分别列出决策与检索 Agent 调用数，避免隐藏补检开销。
